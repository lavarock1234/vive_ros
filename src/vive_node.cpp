...
//        std::cout << static_cast<std::bitset<64> >(state.ulButtonTouched) << std::endl;
        ...
        if(!odom_pubs_.count("controller_" + cur_sn)) {
          odom_pubs_["controller_" + cur_sn] = nh_.advertise<nav_msgs::Odometry>("/vive/controller_"+cur_sn+"/odom", 10);
        }
        nav_msgs::Odometry odom;
        odom.header.stamp = ros::Time::now();
        odom.header.frame_id = "world_vive";
        tf::poseTFToMsg(tf, odom.pose.pose);
        odom_pubs_["controller_" + cur_sn].publish(odom);
      }
      // It's a tracker
      if (dev_type == 3)
      {
